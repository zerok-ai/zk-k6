...
func (r *TestRunReconciler) reconcile(ctx context.Context, req ctrl.Request, log logr.Logger, k6 v1alpha1.TestRunI) (ctrl.Result, error) {
	// Example: Attempting to marshal some data to JSON
    data := map[string]string{"example": "value"}

    // Log before marshaling
    log.Info("Before marshaling", "data", data)
    ...
    // Log after marshaling
    log.Info("After marshaling", "json", string(jsonData))
	var err error
